...
#include "prog.skel.h"   // generado por bpftool gen skeleton
...
int main() {
    ...
    // 2) Configurar perf_event (1kHz por CPU)
    ...
    for (int cpu = 0; cpu < ncpus; cpu++) {
        pfd[cpu] = perf_event_open(&attr, -1, cpu, -1, 0);
        if (pfd[cpu] < 0) {
